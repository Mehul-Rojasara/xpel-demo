      ...
      <Container>
        <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 sm:gap-8 md:gap-14 Xxl:gap-20`}>
          {services.map((service) => (
            <article
              key={service.id}
              className="text-left group cursor-pointer flex-col md:flex-row"
              role="button"
              tabIndex={0}
              aria-label={`${service.title} - ${service.ctaText}`}
            >
              {/* Icon - exact Figma specifications (84x84) */}
              <div className="flex justify-center w-16 h-16 sm:w-20 lg:w-[84px] sm:h-20 lg:h-[84px] sm:min-w-20 lg:min-w-[84px] items-center transition-transform duration-300 group-hover:scale-110">
                {service.iconClass && (
                  <i className={`${service.iconClass} text-3xl sm:text-4xl lg:text-[3.125rem] text-white`}></i>
                )}
              </div>
              <div className='flex flex-col items-center md:items-start'>
                {/* Title - exact Figma typography: Futura Medium 24px, 110% line height, -1% letter spacing */}
                <h3 
                  className="para-large leading-[120%] tracking-tight font-display text-white mb-3 text-center md:text-left"
                >
                  {service.title}
                </h3>
                ...
                <p 
                  className="mb-2 lg:mb-3 para-xsmall leading-relaxed tracking-wide text-white opacity-90 font-sans text-center md:text-left"
                >
                  {service.description}
                </p>
                ...
                {service.ctaText && service.ctaHref && (
                  <a
                    ...
                    className="inline-flex items-center text-white font-display para-small leading-[20px] tracking-normal transition-colors duration-300 "
                  >
                    {service.ctaText}
                    <svg 
